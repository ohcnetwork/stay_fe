...
export default function ViewBooking({ id }) {
    ...
    const availableFilters = {
        ...
        SORT_CHECKIN_ASCENDING: (el, status) => (
            status? el.sort((a, b) => (new Date(a.checkin)) - (new Date(b.checkin))): el.sort((a, b) => (new Date(b.checkin)) - (new Date(a.checkin)))
        ),
        SHOW_UPCOMING: (el, status) => (
            status? el.filter(e => new Date(e.checkin) >= (new Date - (1000 * 60 * 60 * 24))): el.filter(e => new Date(e.checkin) < (new Date - (1000 * 60 * 60 * 24)))
        )
    }
