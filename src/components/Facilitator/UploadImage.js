...
export default function UploadImage({ setFiles, limit = 5, prevImages = [], formLoading = false }) {
    ...
    const handleChange = (e) => {
        if (!formLoading) {
            ...
            files.forEach(file => {
                ...
                setImages({
                    preview: [...images.preview, ...currentImages.map(file =>URL.createObjectURL(file))],
    ...
    return (
        <div className="flex flex-col w-full bg-grey-lighter">
            <div className="flex flex-wrap items-center">
                <label className={`${images.preview.length >= limit? "hidden": "flex"} justify-center items-center px-2 py-1 rounded tracking-wide border border-blue cursor-pointer text-white ${formLoading ? "bg-gray-600 cursor-defaults" : "bg-indigo-600 hover:bg-indigo-800"}`}>
                    <svg
                        className="w-5 h-4"
                        fill="currentColor"
                    ...
                    <input type="file" className="hidden" onChange={handleChange} disabled={formLoading} multiple />
                </label>
